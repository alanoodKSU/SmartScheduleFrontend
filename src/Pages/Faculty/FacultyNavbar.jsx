...
import { FaTable, FaCommentDots, FaBell } from "react-icons/fa";
...
export default function StudentLinksBar() {
  ...
  // ✅ Fetch notifications for current faculty user
  const fetchNotifications = async () => {
    ...
    try {
      ...
      console.log("🔔 Faculty Notifications fetched:", data);
  ...
  const links = [
    { path: "/faculty/schedule", label: "Home", icon: <FaTable /> },
    { path: "/faculty/feedback", label: "Feedback", icon: <FaCommentDots /> },
  ];
    ...
    >
      ...
      <Dropdown align="end" show={showDropdown} onToggle={toggleDropdown}>
        <Dropdown.Toggle
          ...
          id="faculty-notifications-dropdown"
        ...
        >
          ...
          {loading ? (
            ...
            notifications.map((n) => (
              <Dropdown.ItemText
                key={n.id}
                className={`d-block px-3 py-2 ${!n.is_read ? "fw-bold" : ""}`}
              >
